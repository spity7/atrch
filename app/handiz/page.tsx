...
async function Handiz() {
  const handiz_res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/handiz`, {
    cache: "no-store",
  });
  const { handiz } = await handiz_res.json();
  const handizz = handiz?.[0]; // first handiz in DB

  if (!handizz) {
    return <p>No handiz found.</p>;
  }
      ...
      <div id="content" className="site-content">
        <div className="content-holder center-relative content-1170">
          ...
          <div className="one_half last">
            <div
              dangerouslySetInnerHTML={{ __html: handizz.description }}
            ></div>
          </div>
